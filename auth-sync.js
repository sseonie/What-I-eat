...
function statePayload() { return { days, categories, foods, recipes, recipeCategories, importVersion: IMPORT_VERSION }; }
const IMPORT_VERSION = '2026-food-log-3';
const LOCAL_IMPORT_KEY = 'food-diary-' + IMPORT_VERSION + '-done';
...
const SNACK_IMPORT = {
  '2026-08-01':'蛋糕', '2026-08-03':'抹茶拿铁 bql', '2026-08-04':'糖果、串',
  '2026-08-05':'蛋糕', '2026-08-06':'抹茶 gelato', '2026-08-07':'糖果',
  '2026-08-08':'糖果', '2026-08-09':'巧克力', '2026-08-10':'蛋白棒',
  '2026-08-11':'蛋白棒', '2026-08-12':'巧克力 + 饼干', '2026-08-13':'巧克力',
  '2026-08-14':'糖果', '2026-08-15':'巧克力', '2026-08-16':'LeTAO 蛋糕',
  '2026-08-17':'花生蛋白棒 + 糖果', '2026-08-18':'香蕉蛋白棒 + 糖果',
  '2026-08-19':'蛋糕 + 蛋白棒 + 巧克力', '2026-08-20':'希腊酸奶 + 蓝莓',
  '2026-08-21':'蛋白棒 + 糖果（蛋白棒不算…🙈）',
  '2026-08-22':'巧克力 + 饼干（whatever 放弃了）',
  '2026-08-23':'蛋白棒 + 巧克力', '2026-08-24':'焦糖希腊酸奶',
  '2026-08-25':'PHD 蛋白棒 + 巧克力', '2026-08-26':'Quest 蛋白棒',
  '2026-08-27':'巧克力 + 饼干 + 酸奶', '2026-08-29':'PBJ 蛋白棒 + 巧克力',
  '2026-08-30':'酸奶', '2026-08-31':'巧克力 + 蛋白棒',
  '2026-09-01':'饼干 + 巧克力', '2026-09-02':'巧克力',
  '2026-09-03':'巧克力 + 饼干', '2026-09-04':'蛋白棒 + 巧克力',
  '2026-09-05':'饼干 + 巧克力', '2026-09-06':'蛋白棒', '2026-09-07':'巧克力'
};
Object.entries(SNACK_IMPORT).forEach(([date, snacks]) => {
  BULK_IMPORT.days[date] = { ...(BULK_IMPORT.days[date] || {}), snacks };
});
...
async function downloadCloudState() {
  ...
  if (error) { console.warn('Could not load synced diary:', error.message); if (!localStorage.getItem(LOCAL_IMPORT_KEY)) { applyBulkImport({}); saveLocally(); refreshAllViews(); localStorage.setItem(LOCAL_IMPORT_KEY, 'done'); showSyncMessage('早餐、零食和食谱已导入'); } else { showSyncMessage('云端同步尚未完成'); } return; }
  ...
  if (saved.recipeCategories) recipeCategories = saved.recipeCategories;
  const imported = applyBulkImport(saved);
  if (imported) localStorage.setItem(LOCAL_IMPORT_KEY, 'done');
  saveLocally();
  refreshAllViews();
